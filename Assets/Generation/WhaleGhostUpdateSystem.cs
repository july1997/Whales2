...
public class WhaleGhostUpdateSystem : JobComponentSystem
{
    [BurstCompile]
...
#endif
        [ReadOnly] public ArchetypeChunkBufferType<WhaleSnapshotData> ghostSnapshotDataType;
...
#endif
                WhaleSnapshotData snapshotData;
...
#pragma warning restore 649
    ...
    protected override void OnCreate()
    {
        ...
        {
            All = new []{
                ComponentType.ReadWrite<WhaleSnapshotData>(),
        ...
        {
            All = new []{
                ComponentType.ReadOnly<WhaleSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
        ...
        RequireForUpdate(GetEntityQuery(ComponentType.ReadWrite<WhaleSnapshotData>(),
            ComponentType.ReadOnly<GhostComponent>()));
    }
    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        if (!m_predictedQuery.IsEmptyIgnoreFilter)
        {
            var updatePredictedJob = new UpdatePredictedJob
            {
                ...
                ghostSnapshotDataType = GetArchetypeChunkBufferType<WhaleSnapshotData>(true),
...
public partial class WhaleGhostSpawnSystem : DefaultGhostSpawnSystem<WhaleSnapshotData>
{
    struct SetPredictedDefault : IJobParallelFor
    {
        [ReadOnly] public NativeArray<WhaleSnapshotData> snapshots;
    ...
    protected override JobHandle SetPredictedGhostDefaults(NativeArray<WhaleSnapshotData> snapshots, NativeArray<int> predictionMask, JobHandle inputDeps)
    {
